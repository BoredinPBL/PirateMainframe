...
#include <piratemainframe/RawControl.h> // Custom ROS msg
...
int main(int argc, char **argv)
{
  ...
  ros::Publisher control_data_pub = n.advertise<piratemainframe::RawControl>("ctrl_data", 1000);
  ...
  while (ros::ok())
  {
    ...
    piratemainframe::RawControl msg; // Msg to use for stick vals

    // Grab the stick values
    GamepadStickXY(GAMEPAD_1, STICK_LEFT, &stick_lx, &stick_ly);
    GamepadStickXY(GAMEPAD_1, STICK_RIGHT, &stick_rx, &stick_ry);
